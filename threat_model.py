...
def create_threat_model_prompt(app_type, authentication, internet_facing, sensitive_data, app_input):
    prompt = f"""
Act as a cyber security expert with certified threat modelling more than 20 years experience of using the CIAAN threat modelling methodology to produce comprehensive threat models for a wide range of applications. Your task is to analyze the provided code summary, README content, and application description to produce a list of specific threats for the application based on owasp top 10, nist, cis. create in bahasa indonesia akan tetapi istilah dalam cyber nya  masih dalam bahasa inggris. ubah kata "penyerang" jadi "fraudster". buat lebih technical. kata seperti "intercept" atau istilah lainya jangan diubah ke bahasa indonesia.

Pay special attention to the README content as it often provides valuable context about the project's purpose, architecture, and potential security considerations. and provide information about how fraudster works on this case

For each of the CIAAN categories (CONFIDENTIAL, INTEGRITY, AUTHENTICATION, Availability, Non-repudiation,), list multiple (3 or 4) credible threats if applicable. Each threat scenario should provide a credible scenario in which the threat could occur in the context of the application. It is very important that your responses are tailored to reflect the details you are given.
